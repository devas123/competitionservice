...
object CompetitionApiActor {
  ...
  case class Live(cassandraZioSession: CassandraZioSession) extends ActorContext {
    ...
    implicit val managedCompetitionService: ManagedCompetitionService[LIO] = ManagedCompetitionsOperations.live(cassandraZioSession)
  }

  case class Test(competitions: Ref[Map[String, ManagedCompetition]], competitionProperties: Option[Map[String, CompetitionProperties]] = None,
    ...
    implicit val competitionQueryOperations: CompetitionQueryOperations[LIO] = CompetitionQueryOperations.test(
      competitionProperties, categories, competitors, fights, periods, registrationPeriods, registrationGroups, stages
    )
    implicit val managedCompetitionService: ManagedCompetitionService[LIO] = ManagedCompetitionsOperations.test(competitions)
  }
  ...
  final case class GetCompetitors(competitionId: String, searchString: Option[String], pagination: Option[Pagination]) extends ApiCommand[List[Competitor]]
  ...
  def behavior[R: Tag](ctx: ActorContext): ActorBehavior[R with Logging, ActorState, ApiCommand] =
    new ActorBehavior[R with Logging, ActorState, ApiCommand] {
      ...
      override def receive[A](
        ...
        case GetDefaultRestrictions => ZIO.effect(DefaultRestrictions.restrictions.map(DtoMapping.mapRestriction)).map(r => (state, r.asInstanceOf[A]))
        ...
        case GetCompetitionProperties(id) => CompetitionQueryOperations[LIO].getCompetitionProperties(id)
          .map(res => (state, res.asInstanceOf[A]))
        case GetCompetitionInfoTemplate(competitionId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetSchedule(competitionId) => CompetitionQueryOperations[LIO].getPeriodsByCompetitionId(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetCompetitors(competitionId, searchString, pagination) => CompetitionQueryOperations[LIO].getCompetitorsByCompetitionId(competitionId)(pagination, searchString)
          .map(res => (state, res.asInstanceOf[A]))
        case GetCompetitor(competitionId, competitorId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetDashboard(competitionId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetMats(competitionId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetMat(competitionId, matId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetMatFights(competitionId, matId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetRegistrationInfo(competitionId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetCategories(competitionId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetFightById(competitionId, categoryId, fightId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetCategory(competitionId, categoryId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetFightsByMats(competitionId, categoryId, limit) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetFightResulOptions(competitionId, categoryId, fightId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetStagesForCategory(competitionId, categoryId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetStageById(competitionId, categoryId, stageId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
        case GetStageFights(competitionId, categoryId, stageId) => CompetitionQueryOperations[LIO].getCompetitionInfoTemplate(competitionId)
          .map(res => (state, res.asInstanceOf[A]))
      }
    }
}
