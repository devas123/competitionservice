...
object CompetitionApiActor {
  ...
  final case class GetAllCompetitions(override val replyTo: ActorRef[List[ManagedCompetition]])
      extends ApiCommand {
  ...
  )(override val replyTo: ActorRef[List[CategoryDescriptorDTO]]) extends ApiCommand {
    override type responseType = List[CategoryDescriptorDTO]
  }

  final case class GetCompetitionProperties(
    id: String)(
  ...
  final case class GetCompetitionInfoTemplate(competitionId: String)(
    override val replyTo: ActorRef[String]
  ) extends ApiCommand {
    override type responseType = String
  }

  final case class GetSchedule(competitionId: String)(
    override val replyTo: ActorRef[ScheduleDTO]
  ) extends ApiCommand {
  ...
  )(override val replyTo: ActorRef[PageResponse[CompetitorDTO]]) extends ApiCommand {
    override type responseType = PageResponse[CompetitorDTO]
  }

  final case class GetCompetitor(competitionId: String, competitorId: String)(override val replyTo: ActorRef[Option[CompetitorDTO]]) extends ApiCommand {
    override type responseType = Option[CompetitorDTO]
  }

  final case class GetDashboard(competitionId: String)(override val replyTo: ActorRef[List[Period]]) extends ApiCommand {
    override type responseType = List[Period]
  }

  final case class GetMats(competitionId: String)(override val replyTo: ActorRef[List[MatDescriptionDTO]]) extends ApiCommand {
    override type responseType = List[MatDescriptionDTO]
  }

  final case class GetPeriodMats(competitionId: String, periodId: String)(override val replyTo: ActorRef[MatsQueryResult]) extends ApiCommand {
    override type responseType = MatsQueryResult
  }

  final case class GetMat(competitionId: String, matId: String)(override val replyTo: ActorRef[Option[MatDescriptionDTO]]) extends ApiCommand {
    override type responseType = Option[MatDescriptionDTO]
  }

  final case class GetMatFights(competitionId: String, matId: String)(override val replyTo: ActorRef[MatFightsQueryResult]) extends ApiCommand {
    override type responseType = MatFightsQueryResult
  }

  final case class GetRegistrationInfo(competitionId: String)(override val replyTo: ActorRef[Option[RegistrationInfo]]) extends ApiCommand {
    override type responseType = Option[RegistrationInfo]
  }

  final case class GetCategories(competitionId: String)(override val replyTo: ActorRef[List[CategoryStateDTO]]) extends ApiCommand {
    override type responseType = List[CategoryStateDTO]
  }

  final case class GetFightById(competitionId: String, categoryId: String, fightId: String)(override val replyTo: ActorRef[Option[FightDescriptionDTO]])
      extends ApiCommand {
    override type responseType = Option[FightDescriptionDTO]
  }
  final case class GetFightIdsByCategoryIds(competitionId: String)(override val replyTo: ActorRef[Option[Map[String, List[String]]]]) extends ApiCommand {
    override type responseType = Option[Map[String, List[String]]]
  }

  final case class GetCategory(competitionId: String, categoryId: String)(override val replyTo: ActorRef[Option[CategoryStateDTO]]) extends ApiCommand {
    override type responseType = Option[CategoryStateDTO]
  }

  final case class GetPeriodFightsByMats(competitionId: String, periodId: String, limit: Int)(override val replyTo: ActorRef[Map[String, List[String]]])
      extends ApiCommand {
    override type responseType = Map[String, List[String]]
  }

  final case class GetFightResulOptions(competitionId: String, categoryId: String, stageId: String)(override val replyTo: ActorRef[List[FightResultOptionDTO]])
      extends ApiCommand {
    override type responseType = List[FightResultOptionDTO]
  }

  final case class GetStagesForCategory(competitionId: String, categoryId: String)(override val replyTo: ActorRef[List[StageDescriptorDTO]])
      extends ApiCommand {
    override type responseType = List[StageDescriptorDTO]
  }
  final case class GetStageById(competitionId: String, categoryId: String, stageId: String)(override val replyTo: ActorRef[Option[StageDescriptorDTO]])
      extends ApiCommand {
    override type responseType = Option[StageDescriptorDTO]
  }
  final case class GetStageFights(competitionId: String, categoryId: String, stageId: String)(override val replyTo: ActorRef[List[FightDescriptionDTO]])
      extends ApiCommand {
  ...
  def behavior[R: Tag](ctx: ActorContext): ActorBehavior[R with Logging, ActorState, ApiCommand] =
    (_: Context[ApiCommand], _: ActorConfig, state: ActorState, command: ApiCommand, _: Timers[R with Logging, ApiCommand]) => {
      ...
      for {
        ...
        res <- command match {
          ...
          case c: GetDefaultRestrictions => ZIO.effect(DefaultRestrictions.restrictions).flatMap(res => c.replyTo ! res)
            .as(state)
          case c: GetDefaultFightResults => ZIO.effect(FightResultOptionDTO.values.asScala).flatMap(res => c.replyTo ! res.toList)
            .as(state)
          case c: GetAllCompetitions => ManagedCompetitionsOperations.getActiveCompetitions[LIO].flatMap(res => c.replyTo ! res)
            .as(state)
          ...
          case c@GetCompetitor(competitionId, competitorId) => CompetitionQueryOperations[LIO]
            ...
            .flatMap(res => c.replyTo ! res.map(DtoMapping.toDtoCompetitor))
            .as(state)
          case c@GetDashboard(competitionId) => CompetitionQueryOperations[LIO].getPeriodsByCompetitionId(competitionId)
            .flatMap(res => c.replyTo ! res)
            .as(state)
          ...
          case c@GetFightIdsByCategoryIds(competitionId) => FightQueryOperations[LIO]
            ...
            .flatMap(res => c.replyTo ! Option(res))
            .as(state)
          ...
          case c@GetPeriodMats(competitionId, periodId) =>
            ...
            optionRes.value
              .flatMap(res => c.replyTo ! res.getOrElse(MatsQueryResult(List.empty, List.empty)))
              .as(state)
          ...
          case c@GetStagesForCategory(competitionId, categoryId) => CompetitionQueryOperations[LIO]
            ...
            .flatMap(res => c.replyTo ! res.map(DtoMapping.toDtoStageDescriptor))
            .as(state)
