...
case class ActorsShutdownWatcher[R](timeout: Duration = 5.seconds, promise: Promise[Throwable, Unit]) extends ActorBehavior[R with Clock, Set[ActorRef[Nothing]], WatcherMessages] {
...
object ActorsShutdownWatcher {
  ...
  def apply[R](provider: ActorRefProvider, children: Set[ActorRef[Nothing]], promise: Promise[Throwable, Unit]): ZIO[R with Clock, Throwable, ActorRef[WatcherMessages]] =
