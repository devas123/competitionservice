...
object FightsGenerateService {
  ...
  def createScores(ids: List[String], refTypes: List[FightReferenceType]): CanFail[List[CompScoreDTO]] = {
    if (!(ids.size == refTypes.size || refTypes.size == 1)) Left {
      Errors.InternalError(Some("The sizes of ids and refTypes should match, or there should be exactly one refType."))
    }
    else Right {
  ...
  private def createConnectedTripletsFrom(
  ...
  def generateEmptyWinnerRoundsForCategory(
    ...
    val iteration = (
      ...
      if (currentRound >= totalRounds) { Eval.now(Right(Right(result))) }
      else if (currentRound < 0) { Eval.now(Right(Right(result))) }
      else {
        ...
        if (currentRound == 0) {
          ...
          } yield Eval.later(res.map(f => Right(f)))
          either.fold(er => Eval.now(Right(Left(er))), identity)
        }
      }
    }
