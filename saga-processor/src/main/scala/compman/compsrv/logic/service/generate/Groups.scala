...
object Groups {
  ...
  def createGroupFights(competitionId: String, categoryId: String, stageId: String, groupId: String, duration: BigDecimal, competitors: List[CompetitorDTO]): List[FightDescriptionDTO] = {
    val combined = createPairs(competitors)
    combined.filter { it => it._1.getId != it._2.getId }.distinctBy { it => SortedSet(it._1.getId, it._2.getId).mkString }
      .zipWithIndex
      .map { e =>
        val (comps, ind) = e
  ...
  def validateFights(fights: List[FightDescriptionDTO]): CanFail[List[FightDescriptionDTO]] = {
    for {
      _ <- assertE(fights.forall { it => it.scoresSize == 2 }, Some("Some fights do not have scores. Something is wrong."))
      _ <- assertE(fights.forall { it => it.scores.exists(_.forall(_.hasCompetitorIdOrPlaceholderId)) }, Some("Not all fights have placeholders or real competitors assigned."))
    } yield fights
  }

  def distributeCompetitors(competitors: List[CompetitorDTO], rawFights: List[FightDescriptionDTO]): CanFail[List[FightDescriptionDTO]] = {
    for {
      ...
      fightsWithDistributedCompetitors <- competitors.zip(placeholders).traverse { pair =>
        ...
        for {
          fight <- fights.find(f => f.hasPlaceholder(placeholderId)).toRight(Errors.InternalError(Some(s"Cannot find fight for placeholder $placeholderId")))
          updated <- fight.pushCompetitorToPlaceholder(competitor.getId, placeholderId).toRight(Errors.InternalError(Some(s"Cannot add competitor ${competitor.getId} to placeholder $placeholderId")))
        } yield updated
      }
  ...
  def generateStageFights(competitionId: String,
                          categoryId: String,
    ...
    for {
      _ <- assertE(stage.groupsNumber > 0,
        Some(s"Group descriptors are empty (${stage.groupsNumber})"))
      comps: List[CompetitorDTO] = stage.getStageOrder.toInt match {
        ...
        case _ =>
          if (stage.getInputDescriptor.getNumberOfCompetitors <= competitors.size) {
            ...
            competitors ++ generatePlaceholderCompetitorsForGroup(stage.getInputDescriptor.getNumberOfCompetitors - competitors.size)
          }
      }
      totalCapacity = stage.getGroupDescriptors.map(_.getSize.toInt).sum
      _ <- assertE(totalCapacity == comps.size,
      ...
      fights = stage.getGroupDescriptors.foldLeft(0 -> List.empty[FightDescriptionDTO]) { (acc, groupDescriptorDTO) =>
        (acc._1 + groupDescriptorDTO.getSize) -> (acc._2 ++ createGroupFights(competitionId, categoryId, stage.getId, groupDescriptorDTO.getId, duration,
          comps.slice(acc._1, acc._1 + groupDescriptorDTO.getSize)))
      }._2
      res <- validateFights(fights)
    } yield res
  }

  def buildStageResults(stageStatus: StageStatus,
            ...
            case Some(true) =>
              fight.competitors.foreach { it =>
                competitorPointsMap.updateWith(it) { u =>
                  ...
                  Some((pointsDescriptor.map(_.getWinnerPoints.toInt).getOrElse(0) + basis._1, pointsDescriptor.map(_.getWinnerAdditionalPoints.toInt).getOrElse(0) + basis._2, basis._3))
                }
              }
            case _ =>
              ...
              fight.winnerId.map { it =>
                competitorPointsMap.updateWith(it) { u =>
                  val basis = u.getOrElse((0, 0, fight.getGroupId))
                  Some((pointsDescriptor.map(_.getLoserPoints.toInt).getOrElse(0) + basis._1, pointsDescriptor.map(_.getLoserAdditionalPoints.toInt).getOrElse(0) + basis._2, basis._3))
                }
              }
          }
        }
        competitorPointsMap.toList
          .sortBy { pair => pair._2._1 * 10000 + pair._2._2 }
          .zipWithIndex
          .map { v =>
            val (e, i) = v
