...
object FightServicePoc {
  ...
  private def checkIfFightIsPackedOrCanBePackedEventually[F[_] : Monad](fightId: String, fights: Map[String, FightDescriptionDTO]): F[Boolean] = {
    ...
    def checkIfFightCanProduceReference(fightId: String, referenceType: FightReferenceType): Boolean = {
      ...
      val parentFights = fightScores.map(_.map(it => {
        it.getParentReferenceType -> it.getParentFightId
      ...
      if (!fightScoresWithCompetitors.exists(_.length > 0)) {
        referenceType match {
          case FightReferenceType.WINNER =>
            parentFights.exists(arr => arr.exists(it => {
              checkIfFightCanProduceReference(it._2, it._1)
            }))
          case _ =>
            parentFights.exists(it => it.count(tt => {
              checkIfFightCanProduceReference(tt._2, tt._1)
            }) >= 2)
        }
      }
        ...
        referenceType match {
          case FightReferenceType.WINNER =>
            true
          case _ =>
            parentFights.exists(_.exists(it => {
              checkIfFightCanProduceReference(it._2, it._1)
            }))
        }
      }
