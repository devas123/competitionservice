...
object CreateCompetitionProc {
  def apply[F[+_]: Monad: IdOperations: EventOperations, P <: Payload](
    ...
    case x @ CreateCompetitionCommand(_, _, _) =>
      process(x, state)
  }
  ...
  private def process[F[+_]: Monad: IdOperations: EventOperations](
    ...
    val eventT: EitherT[F, Errors.Error, Seq[EventDTO]] =
      for {
        ...
        event <- EitherT.liftF[F, Errors.Error, EventDTO](
          CommandEventOperations[F, EventDTO, EventType].create(
            ...
            payload = Some(
              new CompetitionCreatedPayload()
                .setReginfo(Option(payload.getReginfo).orElse(state.registrationInfo).orNull)
                .setProperties(
                  Option(payload.getProperties).orElse(state.competitionProperties).orNull
                )
            )
          )
        )
