...
object GenerateScheduleProc {
  ...
  private def process[F[+_]: Monad: IdOperations: EventOperations](
    ...
    val eventT: EitherT[F, Errors.Error, Seq[EventDTO]] = for {
      ...
      timeZone <- EitherT.fromOption[F](state.competitionProperties.flatMap(cp => Option(cp.getTimeZone)), Errors.InternalError("No time zone"))
      ...
      scheduleAndFights <- EitherT(ScheduleService.generateSchedule[F](competitionId, periods.toIndexedSeq, mats, stageGraph, timeZone))
      fightUpdatedEvents <-
        EitherT.liftF[F, Errors.Error, List[EventDTO]](scheduleAndFights._2.grouped(100).toList.traverse(fights =>
      ...
      scheduleGeneratedEvent <- EitherT.liftF[F, Errors.Error, EventDTO](
        CommandEventOperations[F, EventDTO, EventType].create(
  ...
  private def getStageGraph(state: CompetitionState): CanFail[StageGraph] = {
    ...
    val stages = categories.values.flatMap { it => state.stages.map(_.values.filter(_.getCategoryId == it.getId)).getOrElse(Seq.empty) }
