...
object FightEditorApplyChangesProc {
  ...
  private def process[F[+_]: Monad: IdOperations: EventOperations](
    ...
    val eventT: EitherT[F, Errors.Error, Seq[EventDTO]] = for {
      ...
      event <-
        if (!stageExists) {
          EitherT.fromEither[F](Left[Errors.Error, EventDTO](Errors.StageDoesNotExist(payload.getStageId)))
        } else {
          for {
  ...
  private def createPayload[F[+_]: Monad](payload: FightEditorApplyChangesPayload, state: CompetitionState) = {
    (for {
      ...
      cleanedUpdates = rawUpdates.map(f =>
        f._1 -> (if (f._2.getStatus == FightStatus.UNCOMPLETABLE) f._2.setStatus(FightStatus.PENDING) else f._2)
      )
    ...
    } yield new FightEditorChangesAppliedPayload().setNewFights(additions.values.toArray)
      .setUpdates(updates.values.toArray).setRemovedFighids(removals.keys.toArray)).value
  }
  ...
  private def clearAffectedFights(
    ...
    if (changedIds.isEmpty) { fights }
      ...
      val affectedFights = fights.filter(fg => {
        fg._2.getScores.exists(s => { changedIds.contains(s.getParentFightId) })
      })
      clearAffectedFights(
        fights.map(f => {
          f._1 -> f._2.setScores(f._2.getScores.map(it =>
            if (changedIds.contains(it.getParentFightId)) it.setCompetitorId(null) else it
          ))
        }),
    ...
    bracketsType match {
      case BracketType.GROUP =>
        ...
        groups.flatMap(gr => {
          ...
          val groupFights  = fightsByGroupId.getOrElse(gr.getId, Iterable.empty).map(f => (f.getId, f)).toMap
          groupChanges.sortBy(c => changePriority(c.getChangeType)).foldLeft(groupFights)((acc, ch) => {
        ...
        }).sortBy(it => { it._2.getNumberInRound }).zipWithIndex.map(z => { z._1._2.setNumberInRound(z._2) })
          .map(f => (f.getId, f)).toMap
      case _ => updateEliminationBrackets(allStageFights, payload.getBracketsChanges)
    }
  }
  ...
  private def updateEliminationBrackets(
    ...
    allStageFights.map(f => {
      f._1 -> bracketChanges.find(change => change.getFightId == f._1).map(change => applyChangeToFight(f._2, change))
        .getOrElse(f._2)
    })
  }

  private def applyChangeToFight(f: FightDescriptionDTO, change: FightsCompetitorUpdated) = {
    if (change.getCompetitors == null || change.getCompetitors.isEmpty) { f.setScores(Array.empty) }
    else {
      val scores = Option(f.getScores).getOrElse(Array.empty)
      f.setScores(change.getCompetitors.zipWithIndex.map(z => {
        val (cmpId, index) = z
        scores.find(s => { s.getCompetitorId == cmpId })
          .orElse(scores.find(s => s.getCompetitorId == null).map(_.setCompetitorId(cmpId))).getOrElse(
            new CompScoreDTO().setCompetitorId(cmpId).setScore(createEmptyScore)
              .setOrder(getMinUnusedOrder(scores, index)).setParentReferenceType(FightReferenceType.PROPAGATED)
          )
      }))
    }
  ...
  def removeCompetitorsFromGroup(
    ...
    val actualGroupSize = groupFights.values.toList.mapFilter(_.scores).flatten
      .distinctBy(s => Option(s.getCompetitorId).getOrElse(s.getPlaceholderId)).size

    if (actualGroupSize <= groupDescriptorDTO.getSize) {
      groupFights.map(e => {
        val (k, it) = e
        k -> it.setScores(it.getScores.map(sc => {
          if (sc.getCompetitorId == change.getCompetitorId) { sc.setCompetitorId(null).setScore(createEmptyScore) }
          else { sc }
        }))
      })
    } else { groupFights.filter(it => !it._2.getScores.exists(sc => sc.getCompetitorId == change.getCompetitorId)) }
  }
  ...
  def addCompetitorToGroup(
    ...
    if (
      groupFights
        .exists(e => e._2.getScores != null && e._2.getScores.exists(_.getCompetitorId == change.getCompetitorId))
    ) { groupFights }
    else {
      val flatScores    = groupFights.values.flatMap(it => { Option(it.getScores).getOrElse(Array.empty) })
      val placeholderId = flatScores.find(it => { it.getCompetitorId == null && it.getPlaceholderId != null })
      placeholderId match {
        case Some(value) =>
          ...
          val updatedFights = groupFights.values.map(fight =>
            fight.getId -> fight.setScores(fight.getScores.map(s =>
              if (s.getPlaceholderId == value.getPlaceholderId) s.setCompetitorId(change.getCompetitorId) else s
            ))
          )
        ...
        case None =>
          ...
          val newCompetitorPairs = for { cmp <- groupCompetitors } yield (change.getCompetitorId, cmp)

          val firstFreeNumberInRound = groupFights.values.map(_.getNumberInRound).max
          val extractor = (fight: FightDescriptionDTO) =>
          ...
          val newPlaceholderId = flatScores.map(it => it.getCompetitorId -> it.getPlaceholderId).toMap +
