...
object CommandProcessorMain extends App {
  ...
  def commandProcessorMainBehavior() = Behaviors.setup[MainActorApi] { context =>
    ...
    val snapshotServiceFactory = (path: String) => SnapshotService.live(path)
    kafkaSupervisor ! CreateTopicIfMissing(appConfig.commandProcessor.commandCallbackTopic, KafkaTopicConfig())
    ...
    context.spawn(
      Behaviors.supervise(StatelessCommandProcessor.behavior(
