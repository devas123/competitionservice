...
trait MessageInfoOperations {
  def fillInfo[T <: MessageInfo](target: T, p: JsonParser, node: TreeNode): T = {
    ...
    target.setMetadata(p
      .getCodec
      .treeToValue(node.get("metadata"), classOf[util.LinkedHashMap[String, String]]))
    target
  }

  def setPayload[T <: MessageInfo](target: T, p: JsonParser, node: TreeNode, payloadClass: Option[Class[_ <: Payload]]): T = {
