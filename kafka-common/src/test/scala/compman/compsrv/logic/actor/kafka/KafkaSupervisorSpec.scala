...
object KafkaSupervisorSpec extends DefaultRunnableSpec {
  ...
  val allLayers: ZLayer[Any, Throwable, Clock with Blocking with Logging with Console] = Clock.live ++ Blocking.live ++
    loggingLayer ++ zio.console.Console.live ++ zio.random.Random.live ++
    TestConfig.live(samples0 = 5, repeats0 = 1, retries0 = 1, shrinks0 = 0)
    ...
    testM("Should query and subscribe respecting Kafka Committed offset") {
      ActorSystem("Test").use { actorSystem =>
        for {
          ...
          msgs <- (0 until messagesCount).toList.traverse(i =>
            messageReceiver.expectMessageClass(15.seconds, classOf[MessageReceived]).map(_.isDefined)
              .onExit(_ => Logging.info(s"Received a message first time ${i + 1}"))
          )
          ...
          msgs2 <- (0 until messagesCount).toList.traverse(i =>
            messageReceiver2.expectMessageClass(15.seconds, classOf[MessageReceived]).map(_.isDefined)
              .onExit(_ => Logging.info(s"Received a message ${i + 1}"))
          )
        } yield assert(msgs.filter(a => a))(hasSize(equalTo(messagesCount))) &&
          assert(msgs2.filter(a => a))(hasSize(equalTo(messagesCount)))
      }.provideLayer(allLayers)
    },
