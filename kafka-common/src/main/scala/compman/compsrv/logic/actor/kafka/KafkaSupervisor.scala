...
object KafkaSupervisor {
  ...
  case class CreateTopicIfMissing(topic: String, topicConfig: KafkaTopicConfig)
    extends KafkaSupervisorCommand[Unit]

  case class QueryAsync(topic: String, groupId: String, replyTo: ActorRef[KafkaConsumerApi])
    extends KafkaSupervisorCommand[Unit]

  case class QuerySync(topic: String, groupId: String, promise: Promise[Throwable, Seq[Array[Byte]]], timeout: Duration = 1.seconds)
  ...
  case class PublishMessage(topic: String, key: String, message: Array[Byte])
    extends KafkaSupervisorCommand[Unit]
  ...
  def behavior[R: Tag](
                        brokers: List[String]
                      ): ActorBehavior[KafkaSupervisorEnvironment[R], Option[ActorRef[KafkaPublishActor.KafkaPublishActorCommand]], KafkaSupervisorCommand] =
    new ActorBehavior[KafkaSupervisorEnvironment[R], Option[ActorRef[KafkaPublishActor.KafkaPublishActorCommand]], KafkaSupervisorCommand] {
      override def receive[A](
                             ...
                             ): RIO[KafkaSupervisorEnvironment[R], (Option[ActorRef[KafkaPublishActor.KafkaPublishActorCommand]], A)] = command match {
        ...
        case QuerySync(topic, groupId, promise, timeout) =>
          for {
            ...
            _ <- context.make(
              ...
              ActorConfig(),
              (),
              KafkaQueryAndSubscribeActor.behavior(topic, groupId, queryReceiver, brokers, subscribe = false, query = true)
            )
        ...
        case PublishMessage(topic, key, message) =>
          state.fold(Task(()))(_ ! PublishMessageToKafka(topic = topic, key = key, message = message)).as((state, ().asInstanceOf[A]))
        case CreateTopicIfMissing(topic, topicConfig) =>
          AdminClient.make(AdminClientSettings(brokers)).use(
            _.createTopic(AdminClient.NewTopic(topic, topicConfig.numPartitions, topicConfig.replicationFactor))
          ).as((state, ().asInstanceOf[A]))

      }

      override def init(actorConfig: ActorConfig, context: Context[KafkaSupervisorCommand],
                        initState: Option[ActorRef[KafkaPublishActor.KafkaPublishActorCommand]],
                        timers: Timers[KafkaSupervisorEnvironment[R], KafkaSupervisorCommand]): RIO[KafkaSupervisorEnvironment[R], (Seq[Fiber[Throwable, Unit]], Seq[KafkaSupervisorCommand[Any]],
        Option[ActorRef[KafkaPublishActor.KafkaPublishActorCommand]])]
      = (for {
        publishActor <- initState.map(RIO(_)).getOrElse(context.make("KafkaPublishActor", ActorConfig(), (), KafkaPublishActor.behavior[R]))
      } yield (Seq.empty, Seq.empty, Some(publishActor))).provideSomeLayer[KafkaSupervisorEnvironment[R]](
