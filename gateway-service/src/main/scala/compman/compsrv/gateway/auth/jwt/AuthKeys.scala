...
object AuthKeys {
  ...
  object JwtPrivateKey {
    def make[F[_]: ApplicativeThrow](privateKey: PKCS8, algorithm: JwtAsymmetricAlgorithm): F[JwtPrivateKey] = {
  ...
  object JwtPublicKey {
    ...
    def rsa[F[_]: ApplicativeThrow](publicKey: String, algorithms: Seq[JwtRSAAlgorithm]): F[JwtPublicKey] =
      build[F](publicKey, algorithms, JwtUtils.RSA)

    def ecdsa[F[_]: ApplicativeThrow](publicKey: String, algorithms: Seq[JwtECDSAAlgorithm]): F[JwtPublicKey] =
    ...
    private def build[F[_]: ApplicativeThrow](
      ...
      keyAlgo: String
    ): F[JwtPublicKey] = ParserKey.parsePublicKey[F](publicKey, keyAlgo).map(key => JwtPublicKey(key, algorithms))
  }
  ...
  object ParserKey {
    ...
    private def parseKey(key: String): Array[Byte] = JwtBase64.decodeNonSafe(
      key.replaceAll("-----BEGIN (.*)-----", "").replaceAll("-----END (.*)-----", "").replaceAll("\r\n", "")
        .replaceAll("\n", "").trim
    )
