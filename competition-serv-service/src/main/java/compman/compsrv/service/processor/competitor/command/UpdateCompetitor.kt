...
class UpdateCompetitor(mapper: ObjectMapper, validators: List<PayloadValidator>) : ICommandExecutor<Competitor>, ValidatedCommandExecutor<Competitor>(mapper, validators) {
    override fun execute(
            entity: Competitor?,
            dbOperations: DBOperations,
            command: CommandDTO
    ): AggregateWithEvents<Competitor> = entity?.let {
    ...
    } ?: error(Constants.COMPETITOR_NOT_FOUND)
    ...
    override val commandType: CommandType
        get() = CommandType.UPDATE_COMPETITOR_COMMAND
}