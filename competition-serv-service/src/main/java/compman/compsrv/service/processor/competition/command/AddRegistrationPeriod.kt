...
class AddRegistrationPeriod(mapper: ObjectMapper, validators: List<PayloadValidator>) : ICommandExecutor<Competition>, ValidatedCommandExecutor<Competition>(mapper, validators) {
    override fun execute(
            entity: Competition,
            dbOperations: DBOperations,
            command: CommandDTO
    ): AggregateWithEvents<Competition> = executeValidated<AddRegistrationPeriodPayload>(command) { payload, com ->
        entity to entity.process(payload, com, AbstractAggregateService.Companion::createEvent)
    }.unwrap(command)


    fun Competition.process(payload: AddRegistrationPeriodPayload, com: CommandDTO, createEvent: (CommandDTO, EventType, Payload?) -> EventDTO): List<EventDTO> {
