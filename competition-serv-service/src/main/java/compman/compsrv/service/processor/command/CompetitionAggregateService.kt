...
@Component
class CompetitionAggregateService(private val scheduleService: ScheduleService,
    ...
    private val sendProcessingInfoCommand: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to clusterOperations.createProcessingInfoEvents(command.correlationId, setOf(command.competitionId)).toList())
    }
    ...
    private val updateRegistrationInfoCommand: CommandExecutor<Competition> = { competition, _, command ->
        executeValidated(command, UpdateRegistrationInfoPayload::class.java) { payload, com ->
            listOf(competition to competition.process(payload, com, this::createEvent))
        }.unwrap(command)
    }
    ...
    private val dropAllBrackets: CommandExecutor<Competition> = { competition, _, command ->
        if (competition.properties.bracketsPublished != true) {
            listOf(competition to competition.categories.map { cat -> createEvent(command, EventType.CATEGORY_BRACKETS_DROPPED, command.payload).setCategoryId(cat) })
        } else {
    ...
    private val dropSchedule: CommandExecutor<Competition> = { competition, _, command ->
        if (competition.properties.schedulePublished != true) {
            listOf(competition to listOf(createEvent(command, EventType.SCHEDULE_DROPPED, command.payload)))
        } else {
    ...
    private val generateSchedule: CommandExecutor<Competition> = { competition, db, command ->
        executeValidated(command, GenerateSchedulePayload::class.java) { payload, com ->
            listOf(competition to competition.process(payload, com, scheduleService, getAllBrackets(com.competitionId, db), getNumberOfCompetitorsByCategoryId(com.competitionId, db), this::createEvent))
        }.unwrap(command)
    }

    private val updateProperties: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_PROPERTIES_UPDATED, command.payload!!)))
    }
    private val startCompetition: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_STARTED, CompetitionStatusUpdatedPayload(CompetitionStatus.STARTED))))
    }
    private val stopCompetition: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_STOPPED, CompetitionStatusUpdatedPayload(CompetitionStatus.STOPPED))))
    }
    private val publishCompetition: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_PUBLISHED, CompetitionStatusUpdatedPayload(CompetitionStatus.PUBLISHED))))
    }
    private val unPublishCompetition: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_PUBLISHED, CompetitionStatusUpdatedPayload(CompetitionStatus.UNPUBLISHED))))
    }
    private val deleteCompetition: CommandExecutor<Competition> = { competition, _, command ->
        listOf(competition to listOf(createEvent(command, EventType.COMPETITION_DELETED, null)))
    }
    ...
    override fun getAggregate(command: CommandDTO, rocksDBOperations: RocksDBOperations): Either<List<Competition>, Competition> {
        return when(command.type) {
            CommandType.CREATE_COMPETITION_COMMAND -> {
               ...
               rocksDBOperations.getCompetition(command.competitionId).right()
            }
        }
    }
