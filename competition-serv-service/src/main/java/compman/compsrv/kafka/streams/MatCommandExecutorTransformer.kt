...
class MatCommandExecutorTransformer(stateStoreName: String,
    ...
    override fun updateCorrelationId(currentState: MatState, command: Command): MatState = currentState.copy(correlationId = command.correlationId!!)
    ...
    override fun doTransform(currentState: MatState?, command: Command?): Triple<String?, MatState?, List<EventHolder>?> {
        fun createEvent(type: EventType, payload: Map<String, Any?>?) =
                EventHolder(command!!.correlationId!!, command.competitionId, command.categoryId, command.matId, type, payload)
        return try {
