...
class GlobalCompetitionCommandExecutorTransformer(stateStoreName: String,
    ...
    override fun updateCorrelationId(currentState: CompetitionProperties, command: Command): CompetitionProperties = currentState.copy(correlationId = command.correlationId!!)
    ...
    override fun doTransform(currentState: CompetitionProperties?, command: Command?): Triple<String?, CompetitionProperties?, List<EventHolder>?> {
        fun createErrorEvent(error: String) = EventHolder(command!!.correlationId!!, command.competitionId,
                command.categoryId,
                command.matId,
