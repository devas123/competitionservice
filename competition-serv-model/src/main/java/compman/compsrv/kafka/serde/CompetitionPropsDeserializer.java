 ...
 */
public class CompetitionPropsDeserializer implements Deserializer<CompetitionProperties> {
    ...
    @Override
    public CompetitionProperties deserialize(String topic, byte[] bytes) {
        if (bytes == null)
            return null;

        CompetitionProperties data;
        try {
            data = objectMapper.readValue(bytes, CompetitionProperties.class);
        } catch (Exception e) {
